...
* Each state can have a default state assigned, allowing you to reuse images for unassigned states. For example, if you set the default state for "NervousOn" to "On," the mod will use the image assigned to the "On" state. If no image is assigned to "On," the game will default to its original image.
